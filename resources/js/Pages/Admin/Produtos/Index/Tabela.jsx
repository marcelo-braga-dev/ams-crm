                ...
                <form onSubmit={atualizarEstoque}>
                    <div className="modal-body">
                        ...
                        <Stack direction="row" spacing={1} alignItems="center" alignContent="center" marginBottom={3}>
                            <Switch size="small" onChange={e => setRecontagem(e.target.checked)} checked={recontagem} />
                        ...
                        <div className="row">
                            ...
                            {/*        <Switch checked={incrementarQtd} size="small" onChange={e => setIncrementarQtd(e.target.checked)}/>*/}
...
const ProdutoRow = React.memo(({ item, updateStatus, isFinanceiro, setEstoque, estoque }) => (<tr key={item.id}>
    <td className="text-center" style={{ width: 70 }}>
        <Switch onChange={value => updateStatus(item.id, value.target.checked)} checked={item.status} size="small" />
    </td>
...
const Tabela = ({ isFinanceiro, filtros }) => {
    ...
    const [statusPedidos, setStatusPedidos] = useState(true);
    ...
    return (
        <CardContainer>
            <CardTable title={'Produtos'} icon={<BoxSeam size={22} />} paginate={paginate} setPaginate={setPaginate} paginateDados={paginateDados}
                       btn={
                           <FormControlLabel label="Ativos" control={
                               <Switch size="small" onChange={e => setStatusPedidos(e.target.checked)} defaultChecked={statusPedidos} />}
                           />
                       }
            >
