...
const Page = ({registros}) => {
    return (
        <Layout titlePage="Tarefas" menu="ferramentas" submenu="ferramentas-tarefas">
            <CardContainer>
                <CardBody>
                    <Link label="Cadasrar Tarefa" href={route('consultor.ferramentas.tarefas.create')}/>
                </CardBody>
            </CardContainer>
                        ...
                        <tr className="align-top">
                            <td style={{minWidth: 300}}>
                                {registros?.aberto?.map((dados) => <Card key={dados.id} dados={dados}
                                                                         btn={route('consultor.ferramentas.tarefas.aberto.show', dados.id)}/>)}                            </td>
                            <td style={{minWidth: 300}}>
                                {registros?.atendimento?.map((dados) => <Card key={dados.id} dados={dados}
                                                                              btn={route('consultor.ferramentas.tarefas.atendimento.show', dados.id)}/>)}
                            </td>
                            <td style={{minWidth: 300}}>
                                {registros?.aprovacao?.map((dados) => <Card key={dados.id} dados={dados}
                                                                            btn={route('consultor.ferramentas.tarefas.aprovacao.show', dados.id)}/>)}
                            </td>
                            <td style={{minWidth: 300}}>
                                {registros?.finalizado?.map((dados) => <Card key={dados.id} dados={dados}
                                                                             btn={route('consultor.ferramentas.tarefas.finalizado.show', dados.id)}/>)}
                            </td>
                        </tr>
                        </tbody>
                    </table>
