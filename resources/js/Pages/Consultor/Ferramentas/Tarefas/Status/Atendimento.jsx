...
const Page = ({dados, tarefas}) => {
    const avancarStatus = () => {
        router.post(route('consultor.ferramentas.tarefas.atendimento.update', dados.id), {_method: 'PUT'})
    }

    const alterarStatusItem = (id, status) => {
        router.post(route('consultor.ferramentas.tarefas.alterar-status-item'), {id, status, _method: 'PUT'})
    }
