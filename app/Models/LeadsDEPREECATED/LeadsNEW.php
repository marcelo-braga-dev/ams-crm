...
class LeadsNEW extends Model
{
    ...
    public function criar($dados, string $status, int $setor, ?int $importacao = null, ?int $userId = null)
    {
        try {
            $this->validarCnpjCpf($dados);

            DB::beginTransaction();

            // Criação do lead com os dados principais
            $lead = $this->create([
                'user_id' => $userId,
                'setor_id' => $setor,
                'cnpj' => converterInt($dados['cnpj']),
                'cpf' => converterInt($dados['cpf']),
                'status' => $status,
                'status_data' => now(),
            ]);

            $this->cadastroDadosAdicionais($lead->id, $dados, $importacao);

            DB::commit();
        } catch (\Exception $e) {
            DB::rollBack();
            $this->logErro($e, 'Erro ao criar lead', $userId, $setor, $dados);
            throw new \DomainException($e->getMessage());
        }
    }

    protected function validarCnpjCpf($dados)
    {
        if (empty($dados['cnpj']) && empty($dados['cpf'])) {
            throw new \DomainException('CNPJ ou CPF deve ser informado.');
        }
    }

    protected function logErro(\Exception $e, string $mensagem, ?int $userId, int $setor, $dados)
    {
        Log::error($mensagem . ': ' . $e->getMessage(), [
            'user_id' => $userId,
            'setor_id' => $setor,
            'dados' => $dados,
            'trace' => $e->getTraceAsString(), // Inclui o trace para melhor diagnóstico
        ]);
    }

    public function cadastroDadosAdicionais(int $leadId, $dados, ?int $importacao)
    {
        // Criação de dados adicionais do lead
        (new LeadsDados())->criar($leadId, $dados, $importacao);

        // Criação de dados secundários do lead (opcional)
        if (!empty($dados['secundarios'])) {
            (new LeadsDadosSecundarios())->criar($leadId, $dados['secundarios'], $importacao);
        }

        // Criação de telefones do lead (opcional)
        if (!empty($dados['telefones'])) {
            (new LeadsTelefones())->criar($leadId, $dados['telefones'], $importacao);
        }

        // Criação de endereços do lead (opcional)
        if (!empty($dados['endereco'])) {
            (new LeadsEnderecos())->criar($leadId, $dados['endereco'], $importacao);
        }
        return $dados;
    }
    ...
    public function createLead($dados, $setor, $usuario = null, $importacao = null, $status = false)
    {
        $cnpj = preg_replace('/[^0-9]/', '', $dados['cnpj'] ?? null);
    ...
    private function cadastrarTelefones($id, $telefones): void
    {
        foreach ($telefones ?? [] as $item) {
            if ($item) {
                ...
                (new LeadsTelefones())->criar($id, $telefone);
            }
        }
    }
    ...
    public function getCards($id)
    {
        ...
        return $this->newQuery()
            ...
            ->get(['leads.id', 'status', 'leads.user_id', 'sdr_id', 'nome', 'razao_social', 'cnpj', 'cnpj', 'telefone', 'status_data', 'classificacao',
                ...
                DB::raw('DATEDIFF(CURDATE(), leads.pedido_em) as pedido_dias'),
    ...
    public function getDadosMinimoPaginate($setor, $filtros)
    {
        ...
        if ($filtros['leads'] ?? null) {
            ...
            if ($filtros['leads'] == 'ativados') $query->whereNotNull('pedido_em');
        }
        ...
        $dados = $items->transform(function ($item) use ($nomeConsultores, $setores) {
            return [
                ...
                'infos' => [
                    ...
                    'pedido_data' => $item->pedido_em ? date('d/m/y H:i', strtotime($item->pedido_em)) : null,
                    'pedido_periodo' => $this->difData($item->pedido_em)
                ]
            ];
        });
